...
cur_target.execute("SELECT EXISTS (SELECT 1 AS result FROM pg_tables WHERE schemaname = 'xamplify_test_load' AND tablename = 'xt_xtremandlog_d');")
tableExists = cur_target.fetchone()[0]
if tableExists==True:
    cur_target.execute("""truncate table xamplify_test_load.xt_xtremandlog_d cascade""")
    print("table is truncated")

else:
    cur_target.execute("""CREATE TABLE xamplify_test_load.xt_xtremandlog_d
	 (
    id integer NOT NULL,
    user_id integer,
    campaign_id integer,
    ...
    CONSTRAINT xt_xtremandlog_d_pkey PRIMARY KEY (id)
)    ;
    """)
...
for row in rows:
    cur_target.execute("""insert into xamplify_test_load.xt_xtremandlog_d(id,
    user_id,
    campaign_id,
    action_id,
    session_id,
...
cur_target.execute("select count(*) from xamplify_test_load.xt_xtremandlog_d")
